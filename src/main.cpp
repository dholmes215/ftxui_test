...
// configuration step. It creates a namespace called `ftxui_test`. You can
...
struct GameBoard {
    ...
    GameBoard()
    {
        visit([](const auto x, const auto y, auto& gameboard) {
            gameboard.set(x, y, true);
        });
    }
...
void consequence_game()
{
    ...
    std::uniform_int_distribution<std::size_t> x(static_cast<std::size_t>(0),
                                                 gb.width - 1);
    std::uniform_int_distribution<std::size_t> y(static_cast<std::size_t>(0),
                                                 gb.height - 1);
...
struct Bitmap : ftxui::Node {
    ...
    {
        requirement_ =
            ftxui::Requirement{.min_x = static_cast<int>(width_),
    ...
    void Render(ftxui::Screen& screen) override
    {
        for (std::size_t x = 0; x < width_; ++x) {
            for (std::size_t y = 0; y < height_ / 2; ++y) {
                ...
                p.background_color =
                    ftxui::Color{top_color.R, top_color.G, top_color.B};
                p.foreground_color = ftxui::Color{
...
{
    // this should probably have a `bitmap` helper function that does what you
    ...
    auto game_iteration =
        [&](const std::chrono::steady_clock::duration elapsed_time) {
            ...
            // things
            auto& small_bm_pixel = small_bm->data().at(
                static_cast<std::size_t>(elapsed_time.count()) %
                small_bm->data().size());
    ...
    auto make_layout = [&] {
        ...
        ++counter;
        // we will dispatch to the game_iteration function, where the work
        // happens
 ...
 Options:
        ...
        std::map<std::string, docopt::value> args = docopt::docopt(
            ...
            fmt::format(
                ...
                ftxui_test::cmake::project_version));  // version string,
                                                       // acquired from
                                                       // config.hpp via CMake
